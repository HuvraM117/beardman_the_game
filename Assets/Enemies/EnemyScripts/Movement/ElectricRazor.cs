...
public class ElectricRazor : MonoBehaviour {
	...
	void Update () {
		if (!attacking) {
			...
			if (distanceToPlayer <= attackRange) {
				...
				Debug.Log("Current float distance: " + currentFloated);

				if (currentFloated >= idleFloatRange) {
					Debug.Log ("Auto switch directions!!! Moving Right = " + movingRight);
					currentFloated = 0;
					initialPosition = transform.position;
	...
	private IEnumerator AttackPlayer() {
		...
		while (xDistanceMoved <= xDistance) { 	//NOTE: only takes x distance into account
			...
			xDistanceMoved = Mathf.Abs(startX - transform.position.x);
